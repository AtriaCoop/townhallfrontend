...
export default function Modal({ title, currUserId, onClose, onUserSelect }) {
  ...
  const getProfileImage = (user) => {
    ...
    if (CLOUD_NAME)
      return `https://res.cloudinary.com/${CLOUD_NAME}/${user.profile_image}`;
  ...
  return (
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <p className={styles.subtitle}>{t("messages.chatModalDesc")}</p>
        ...
        <div className={styles.userList}>
          {filteredUsers.length > 0 ? (
            ...
            <p className={styles.noResults}>{t("messages.noMembersFound")}</p>
          )}
        </div>
      </div>
    </div>
