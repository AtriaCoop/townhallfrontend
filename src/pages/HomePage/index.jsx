...
export default function HomePage() {
      ...
      try {
        ...
        const formattedPosts = data.posts
          ...
          .map((p) => ({
            ...
            date: formatDistance(new Date(p.created_at), new Date(), {
              addSuffix: true,
            }),
            ...
            liked_by: p.liked_by || [],
            isLiked: profileData
              ? userInLiked(p.liked_by, profileData.id)
              : false,
            comments: p.comments || [],
  ...
  const getPageNumbers = () => {
    ...
    if (currentPage > endThreshold) {
      return Array.from(
        { length: MAX_PAGINATION_BUTTONS },
        (_, i) => totalPages - MAX_PAGINATION_BUTTONS + i + 1
      );
    }
      ...
      <div className={styles.homeContainer}>
        <div className={styles.title}>
          <div className={styles.headerSection}>
            <h1 className={styles.header}>News Feed</h1>
          ...
          <p>
            A place to share general information and updates with members of the
        ...
        {/* Post component */}
        {loading ? (
          <div className={styles.newsfeedLoader}>
            <Loader />
          </div>
        ) : (
          <>
            {posts.length < 1 ? (
              <div className={styles.emptyFeed}>
                <i>Your feed is empty</i>
              </div>
            ) : (
              posts.map((post) => {
                console.log(
                  "Rendering Post component for post:",
                  post.id,
                  post.fullName
                );
                return (
